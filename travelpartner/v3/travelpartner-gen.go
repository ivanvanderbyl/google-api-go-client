...
type DataIssueDetail struct {
	...
	// characters.
	//   "BRAND_NOT_ALLOWED" - The vacation rental property's brand isn't
	// enabled. Contact your account manager for details.
	//   "FLAGGED_DUE_TO_SUSPICIOUS_METADATA" - Vacation rental listing has
	// been flagged due to suspicious metadata. Contact your account manager
	// for details.
	//   "NOT_ENOUGH_IMAGES_PROVIDED" - Vacation rental properties must have
	// at least one image. Five or more images are recommended.
	//   "IMAGE_PROCESSING_IN_PROGRESS" - The images for this vacation
	// rental property are still being processed, which typically takes up
	// to 2 weeks.
	...
	//   "NON_VR_ACCOMMODATION_TYPE_BASED_ON_LISTING_NAME" - Vacation rental
	// listing is not a vacation rental type based on the name.
...
type ParsedListing struct {
	...
	// Location: Location of listing in lat/lng (degrees).
	...
	// Review: Reviews associated with this listing. Each review proto has a
...
type PropertyPerformanceResult struct {
	...
	// ImpressionCount: The total number of impressions that were recorded
	// for this result. This value is rounded to preserve user privacy.
...
type Review struct {
	...
	// LanguageCode: Language of the review, such as "en", "de", etc.
	LanguageCode string `json:"languageCode,omitempty"`
...
// included in the ParticipationResult. Using an `aggregateBy`
// specification that produces a large number of rows will cause an
// error. This is especially true when aggregating by `partnerHotelId`
// or more than two fields. To reduce the possibiliy of an error, filter
// by `partnerHotelId` and `date` to only include a select number of
// hotels and dates. Accounts with a large number of hotels will need to
// further reduce data with more filtering.
...
// `in` operator. Otherwise, spaces can be omitted. Conditions can be
// joined using the `and` operator. The `date` field is required. All
// other fields are optional. Examples of valid conditions are as
// follows: * `advanceBookingWindow = 2` * `advanceBookingWindow >= 0` *
...
// `lengthOfStayDays <= 5` * `lengthOfStayDays between 1 and 5` *
// `occupancy = 2` * `occupancy >= 0` * `occupancy <= 5` * `occupancy
// between 1 and 5` * `partnerHotelId = 'AAA'` * `partnerHotelId in
// ('AAA', 'BBB')` * `userRegionCode = 'US'` * `userRegionCode in ('US',
// 'CA')`
...
//
// - name: The resource name of price accuracy view being queried.
...
// List: Lists available price accuracy views.
//
//   - parent: The resource name of the account being queried. The format
//     is `accounts/{account_id}`.
...
// must be in YYYY-MM-DD format. The earliest acceptable date is
// 2021-03-09; earlier date values will be coerced to 2021-03-09. Values
// for `partnerPropertyDisplayName` and `brand` are matched
// case-insensitively. Examples of valid conditions are as follows: *
